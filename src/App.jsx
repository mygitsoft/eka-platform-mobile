...
function App() {
    ...
    const loadEmployees = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(`Unable to load employees: ${response.status} ${response.statusText} ${body}`);
        }
  ...
  const loadTodayCount = async () => {
    ...
    try {
      const response = await fetch(`${getApiBaseUrl()}/attendance/today/count`);
      if (!response.ok) {
        const body = await response.text();
        throw new Error(`Unable to load today count: ${response.status} ${response.statusText} ${body}`);
      }
  ...
  const loadTotalEmployeeCount = async () => {
    ...
    try {
      const response = await fetch(`${getApiBaseUrl()}/employee/count`);
      if (!response.ok) {
        const body = await response.text();
        throw new Error(`Unable to load employee count: ${response.status} ${response.statusText} ${body}`);
      }
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (!response.ok) {
        const body = await response.text();
        throw new Error(body || 'Unable to save attendance.');
      }
      ...
      try {
        const response = await fetch(`${getApiBaseUrl()}/today/count`);
        if (response.ok) {
          const data = await response.json();
          if (typeof data === 'number') {
            setTodayCount(data);
